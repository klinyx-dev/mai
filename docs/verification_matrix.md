...
| FR-4 | Add slot with shape and overlap validation | Covered | `core/src/application/scheduler_service.rs` tests (`rejects_overlapping_slots_for_same_resource owner`), `core/src/domain/time_range.rs` tests (`rejects_start_after_end`, `rejects_equals_start_and_end`) |
...
| FR-7 | Add appointment books available slot and derives host from slot | Covered | `core/src/application/scheduler_service.rs` (`booking_creates_exactly_one_appointment_and_marks_slot_booked`, `appointment_host_is_derived_from_slot_resource owner`) |
...
| FR-12 | Resource owner-scoped weekly query filtering | Covered | `core/src/layout/weekly_layout/tests.rs` (`resource owner_filter_limits_slot_projection`, `resource owner_filter_limits_appointment_projection_by_slot_resource owner`, `filtered_projections_are_identical_across_insertion_orders`), adapter boundary checks in `core/tests/serialization_contract.rs` and `core/tests/wasm_web_smoke.rs` |
| FR-13 | Visible-hour window query behavior and validation | Covered | `core/src/layout/weekly_layout/tests.rs` (`visible_window_clips_and_filters_slot_projection`, `visible_window_clips_and_filters_appointment_projection`), `core/src/application/scheduler_service.rs` (`weekly_layout_query_rejects_invalid_visible_window_order`, `weekly_layout_query_rejects_out_of_range_visible_window_bound`), adapter error mapping checks in `core/tests/serialization_contract.rs` and `core/tests/wasm_web_smoke.rs` |
