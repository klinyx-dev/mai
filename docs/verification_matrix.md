...
| FR-5 | Delete slot by ID; reject when booked | Partially Covered | Rejection path is covered in `core/src/application/scheduler_service.rs` (`booked_slot_cannot_be_deleted`); add an explicit success-path test proving available-slot deletion removes the slot by ID. |
| FR-6 | Cancel slot; prevent booking cancelled slots; reject cancel when booked | Partially Covered | Cancelled-slot booking rejection is covered (`cannot_book_cancelled_slot`, `cancel_then_booking_is_rejected`); add an explicit service-level test for cancelling a `booked` slot returning deterministic `slot_not_available`. |
